...
class UIBase extends Base {
    ...
    constructor(view = { html: "<div></div>" }, style = { css: "" }){
        ...
        pView = pView.then(function($el){
            return $el.addClass(this.getId());
        });

        var pStyle, styleIsLess, styleInCache;
        if (style.css || style.less){
            styleIsLess = !!style.less;
            pStyle = when(style.css || style.less);
        } else if (style.url){
